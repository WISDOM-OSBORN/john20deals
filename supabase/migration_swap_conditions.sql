...
-- Adds columns to swap_requests for accepted swap conditions + WhatsApp
-- notification tracking. Copy & run in Supabase SQL Editor.
-- =====================================================================

-- 1. Trade-in value offered for the customer's device (GHS)
ALTER TABLE public.swap_requests ADD COLUMN IF NOT EXISTS trade_in_value NUMERIC;

-- 2. Cash difference the customer pays (or receives) on top of trade-in (GHS)
ALTER TABLE public.swap_requests ADD COLUMN IF NOT EXISTS cash_difference NUMERIC;

-- 3. Free-text terms of the accepted swap (delivery, warranty, etc.)
ALTER TABLE public.swap_requests ADD COLUMN IF NOT EXISTS terms TEXT;

-- 4. Timestamp when admin accepted & sent the WhatsApp notification
ALTER TABLE public.swap_requests ADD COLUMN IF NOT EXISTS notified_at TIMESTAMPTZ;

-- 5. Ensure RLS stays disabled (matches existing tables)
